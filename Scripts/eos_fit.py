...
def equation_of_state(psibarpsi, beta, am, A, betac, p, B, delta):
    ...
    return np.array(A * (beta - betac) * psibarpsi**p + B * psibarpsi**delta -
                    am,
                    dtype=float)
...
def error_on_equation_of_state_bootstrap(psibarpsi, psibarpsiErr, beta, am, A,
    ...
    for i in range(nbootstrap):  # bootstrap with 10 samples
        ...
        eqerrors.append(equation_of_state(pbp, beta, am, A, betac, p, B,
                                          delta))
...
def error_on_equation_of_state_derivs(psibarpsi, psibarpsiErr, beta, am, A,
                                      betac, p, B, delta):
    """Error on Powerlaw equation of state - lhs"""
    return np.array(A * (beta - betac) * p * psibarpsi**(p - 1) +
                    B * delta * psibarpsi**(delta - 1) - am,
                    dtype=float) * psibarpsiErr
...
def residuals(params,
    ...
    return equation_of_state(psibarpsi,beta,am,A,betac,p,B,delta)/\
            error_on_equation_of_state_f(psibarpsi,
...
if Ls == 32:
    ...
    a0 = np.array(
        [3.30445818, 0.25055222, 1.06456595, 32.29585502, 4.64339238])
elif Ls == 48:
    a0 = np.array([3.11646997, 0.25647005, 1.0538188, 11.59937539, 3.94485828])
print("Fitting (this may take a while)...")
output = leastsq(func=residuals,
                 x0=a0,
                 args=(values_and_errors.beta, values_and_errors.psibarpsi,
                       values_and_errors.psibarpsiErr, values_and_errors.mass),
                 full_output=1)
...
plt.title(f"Ls={Ls}, X^2/ndof = {num:.2f}/{den}, b={betac_str},d={delta_str},B={B:.2f}")
...
for mass, beta in psibarpsi.index:
    psibarpsi[(mass, beta)] = brentq(
        f=lambda y: equation_of_state(y, beta, mass, *found_params),
        a=0.0,
        b=0.5)
