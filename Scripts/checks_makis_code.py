...
def get_all_data(basename):
    ...
    # We assume we are in MakisChecks/Analysis
    filenames = glob(f"../Data/*/{basename}") 
...
def read_linecounts(basename):
    ...
    '''
    # We assume we are in MakisChecks/Analysis
    return pd.read_csv(f"../Data/linecount_{basename}.txt", sep = "\s+").set_index("Directory")
    ...
    for basename in ['fort.200', 'fort.11']:
        ...
        for d in get_all_data(basename):
            ...
            for datacolidx in range(trajcolidx+1,data.shape[1]):
                ...
                original_data_len = linecounts.loc[os.path.basename(directory),'ReferenceLength'] 
